...
input.addEventListener("change", () => onFileUpload(input.files));
...
async function onFileUpload(files: FileList) {
    ...
    const tiffTag = [
        ...
        "uint32",
        "value",
        function (ds: DataStream, s) {
            const { position: oldPosition } = ds;

            if (s.count * tiffByteSize[s.type] > 4) {
                ds.seek(ds.readUint32());
            }
            let v: any = { error: "Unknown TIFF Field" };
            switch (s.type) {
                case 1:
    ...
    const jpegStruct = [
        "start",
        function (ds: DataStream) {
            const t = ds.readUint16();
        ...
        [
            ...
            [
                ...
                "data",
                {
                    get: function (ds: DataStream, s) {
                        switch (s.tag) {
                            case 0xffe1: // EXIF
                                const exif = ds.readString(6);
                                ...
                                if (exif == "Exif\0\0") {
                                    ...
                                    const xmp = ds.readCString();

                                    if (xmp == "http://ns.adobe.com/xap/1.0/") {
                                        return {
                                            xmp: xmp,
                                            data: ds.readString(
                                                s.length - 2 - xmp.length - 1
                                            )
                                        };
                                    }
                            ...
                            case 0xffe0: // APP0
                                if (s.length >= 7) {
                                    ...
                                    if (jfif == "JFIF" || jfif == "JFXX") {
                                        ...
                                        const u8 = ds.mapUint8Array(
                                            s.length - 7
                                        );
                                        const rv = new DataStream(
                                            u8,
                                            null,
                                            DataStream.BIG_ENDIAN
                            ...
                            case 0xffda: // image stream
                                ...
                                return {
                                    components: cs,
                                    imageData: ds.mapUint8Array(u8.length - 2)
                                        .length
                                };
                            default:
                                return ds.mapUint8Array(s.length - 2).length;
    ...
    const ds = new DataStream(file);
    ds.endianness = DataStream.BIG_ENDIAN;

    const obj = ds.readStruct(jpegStruct) || parseTIFF(file);
